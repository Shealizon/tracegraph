...
提示词：`prompts/tracegraph-extract.md`（输出 `relation-graph@1` generic 格式，由 `adapter.compileGeneric` 自动推导边）。

## 调用管线（脚本化）

1. 源准备：tex/md 直接用；pdf 先 `pdftotext` 提取文本。源复制到 `.llmsrc/`（见下"坑1"）。
2. 生成：`opencode run --pure -m deepseek/deepseek-v4-pro -f prompts/tracegraph-extract.md -f <src> -- '<指令>' < /dev/null`（见"坑2"）。
...
- **tex 最准**：hardy 24 节点与规则提取器 `tracegraph.json`(24) 完全一致，中心同为 THEOREM 22 "Audit"。
...
## 对话记录留存（LLM 回归测试）
每次 LLM 提取的**原始输出**保存到 `samples/raw/<name>.raw.txt`；提示词版本由 git 追踪 `prompts/tracegraph-extract.md`。二者构成可复现的对话记录，便于回归与提示词迭代。校验用 `scripts/check-sample.mjs`（节点/边/未解析引用/空 sections）。
