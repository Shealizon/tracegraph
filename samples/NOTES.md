...
用 arxiv 三个非数学领域论文（PDF→pdftotext→deepseek）测同一管线/提示词：

| 样例 | 领域 | 主题 | 节点 | 边 | 未解析 |
|------|------|------|------|----|-------|
| bio | q-bio（行为/社科） | nudging 个人/禁令规范 | 23（11命题+12文献） | 27 | 0 |
| med | physics.med-ph（医学影像） | 双能 CT 重建 | 14（1定理+5命题+8文献） | 13 | 0 |
| cl | cs.CL（语音/NLP） | 端到端说话人分离 | 28（6定理+4引理+3命题+15文献） | 41 | 0 |

**结论**：为数学论文设计的提示词对非数学领域**普适性强**——deepseek 把实验论文的假设/发现/方法映射为 proposition（核心结论用 theorem、子结论用 lemma），参考文献为 bib，并自动推导依赖边；命名标识（Intro/Disc/S1、AnNet 等）合理。三篇均 0 未解析、0 KaTeX 错误、布局聚拢、视觉良好。
（纯文学/人文论文 arxiv 稀缺，以 cs.CL 数字语言领域近似。）
