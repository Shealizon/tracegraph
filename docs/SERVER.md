...
`TRACEGRAPH_DATA` 指向持久卷并限制为服务进程可读写。也可运行
...
封闭注册后，管理员可在管理员面板中输入用户名创建普通用户，登录账号就是该用户名。
初始密码为“用户名 + 5 位随机小写字母 + @graph.akusm.com”，
且只在创建完成时显示一次。用户登录后可从账号菜单修改密码。
...
服务器启动时会加载内置及管理员导入的 Skills/Tools 扩展。只有管理员能导入或删除扩展；
导入的 Python 依赖安装在每个扩展自己的虚拟环境中。状态为可用的 Skill 和工具会注入所有
已登录用户的本地模型、云端模型与 Codex 任务。完整包格式、运行协议和安全边界见
...
`/opt/tracegraph`、重启 `tracegraph.service` 并重新加载 Nginx。
