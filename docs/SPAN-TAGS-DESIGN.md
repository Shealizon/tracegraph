# 卡片内文字片段打标设计（Span Tags）

在「整卡片打标」之外，标签成员可指向卡片正文里的**具体文字片段**（包括行内/行间公式），并可一键跳转。
...
"members": [
  "thm:main",                                            // 整卡片（旧，兼容）
  { "node": "thm:main", "type": "span",                  // 文字片段
    "section": "statement", "start": 12, "end": 48,
    "text": "文字与 $x+1$", "offsetMode": "annotation-md" }
]
```

- `type` 缺省/字符串 = `node`。
- 跨文档唯一化（compileProject）时，对象成员的 `node` 字段同样重映射。
- 有序标签里**每个成员**（node/span）按序得到 Step N。
- 历史 `pos` 成员只做兼容读取，不再提供创建入口。
...
- span 贴片与普通贴片**同样式 + 同长按快捷菜单**（全览 / 有序的 ◀▶）。
...
### 打标模式（ctx.tagEditing）
- hover 卡片 → `modal-body`（含 collapsed）上盖一层**半透明遮罩**，提示单击整卡、拖选文字或公式。
- **单击**正文（无选区）→ 整卡片打标（切换 node 成员）。
- **拖动或双击选中文字/公式** → 自动建 `span` 成员（无需再确认）。

### 非打标模式 · 右键正文空白/未选中处 → `menu-blank`（图标+文字）
复制（▸ 二级：所有内容/标题/选中）· 固定 · 关闭为节点 · 隐藏。正文空白处不再创建位置标签。
...
## 5. 跳转
点击 span 贴片 / 全览的成员 → focusNode + 打开卡片 + `scrollIntoView` 到该 Range。

## 实施阶段
A 选区配色 · B 打标遮罩 · C 数据模型(对象成员+remap+全览/focus 适配) · D 选中自动打标/单击整卡 ·
E span 虚线+贴片渲染 · F simple-menu(含插入行) · G menu-blank / 右键选中 menu · H span 贴片长按菜单 + 跳转。
